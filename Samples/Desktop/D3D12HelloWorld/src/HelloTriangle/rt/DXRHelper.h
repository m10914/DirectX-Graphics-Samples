...
inline ID3D12Resource* CreateBuffer(ID3D12Device* m_device, uint64_t size,
                                    ...
                                    const D3D12_HEAP_PROPERTIES& heapProps, UINT64 alignment = 0)
{
  D3D12_RESOURCE_DESC bufDesc = {};
  bufDesc.Alignment = alignment;
...
IDxcBlob* CompileShaderLibrary(LPCWSTR fileName)
{
  ...
  if (!pCompiler)
  {
	  ...
	  ThrowIfFailed(s_pDxcCreateInstanceProc(CLSID_DxcCompiler, __uuidof(IDxcCompiler), (void **)&pCompiler));
	  ThrowIfFailed(s_pDxcCreateInstanceProc(CLSID_DxcLibrary, __uuidof(IDxcLibrary), (void **)&pLibrary));
