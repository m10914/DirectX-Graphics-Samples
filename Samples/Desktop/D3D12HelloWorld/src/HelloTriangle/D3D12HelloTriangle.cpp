...
void D3D12HelloTriangle::CreateShaderBindingTable() {
	...
	UINT64 heapPointer = static_cast<UINT64>(srvUavHeapHandle.ptr);

	// The ray generation only uses heap data
	m_sbtHelper.AddRayGenerationProgram(L"RayGen", { (void*)heapPointer });
...
void D3D12HelloTriangle::CreateRaytracingPipeline()
{
    ...
    // used.
    m_rayGenLibrary = nv_helpers_dx12::CompileShaderLibrary(L"rt/RayGen.hlsl");
    m_missLibrary = nv_helpers_dx12::CompileShaderLibrary(L"rt/Miss.hlsl");
    m_hitLibrary = nv_helpers_dx12::CompileShaderLibrary(L"rt/Hit.hlsl");
...
void D3D12HelloTriangle::OnInit()
{
	...
	ThrowIfFailed(m_commandList->Close()); //moved here so we could do proper RT init
...
void D3D12HelloTriangle::CheckRaytracingSupport()
{
    ...
    if (options5.RaytracingTier < D3D12_RAYTRACING_TIER_1_0)
        throw std::runtime_error("RT not supported");
